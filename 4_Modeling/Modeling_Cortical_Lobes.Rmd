---
title: "Modeling -- Cortical Lobe Averages"
output: 
  html_document:
    css: "../my_style.css"
---
...
The `glmnet` elastic net model predicted the same outcome value (0.3539) for every single test case. Together with the `NaN` $R^2$ values, this suggests there were issues with retained predictor variables. This will be further explored in [Model Evaluation](https://anniegbryant.github.io/DA5030_Final_Project/Pages/5_Evaluation.html) through variable importance analysis.
...
Lastly, I will append the cortical lobe-aggregated dataset and ensemble model developed herein to an .RData file for use in Model Evaluation. First, since all of the objects have the same names across the four modeling pages, I will re-name model objects and datasets to be cortical lobe data-specific:
```{r}
# Rename relevant objects to have Cortex (CTX)-specific names
models.for.ensemble.CTX <- ensemble.models
model.metrics.CTX <- overall.ensemble.results
stacked.ensemble.glm.CTX <- stacked.ensemble.glm
stacked.ensemble.glmnet.CTX <- stacked.ensemble.glmnet
stacked.ensemble.rf.CTX <- stacked.ensemble.rf
train.preds.CTX <- cbind(train.df, select(cortex.train, -CDRSB))
test.preds.CTX <- cbind(test.df, select(cortex.test, -CDRSB))

# If RData file already exists, use resave from cgwtools to append these objects
if (file.exists("../Model_Results.RData")) {
  cgwtools::resave(models.for.ensemble.CTX, model.metrics.CTX,
                   stacked.ensemble.glm.CTX, stacked.ensemble.glmnet.CTX,
                   stacked.ensemble.rf.CTX, train.preds.CTX, test.preds.CTX, 
                   file="../Model_Results.RData")
} else {
  # If the RData file does not yet exist, create it here
  save(models.for.ensemble.CTX, model.metrics.CTX,
       stacked.ensemble.glm.CTX, stacked.ensemble.glmnet.CTX,
       stacked.ensemble.rf.CTX, train.preds.CTX, test.preds.CTX, 
       file="../Model_Results.RData")
}
