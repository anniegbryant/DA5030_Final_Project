...
Lastly, I will append the PCA dataset and ensemble model developed herein to an .RData file for use in Model Evaluation. First, since all of the objects have the same names across the four modeling pages, I will re-name model objects and datasets to be PCA-specific:
```{r}
...
train.preds.PCA <- cbind(train.df, select(pca.train, -CDRSB))
test.preds.PCA <- cbind(test.df, select(pca.test, -CDRSB))
...
if (file.exists("../Model_Results.RData")) {
  cgwtools::resave(models.for.ensemble.PCA, model.metrics.PCA,
                   ...
                   stacked.ensemble.rf.PCA, train.preds.PCA, test.preds.PCA, 
