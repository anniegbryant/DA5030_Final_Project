...
Looking at the mean RMSE across sample iterations, the elastic net (`glmnet`) has the lowest RMSE; by contrast, the neural network has the highest RMSE. In terms of mean $R^2$, the values are all pretty low (all <0.3), ranging from 0.26 (neural network) down to 0.06 (kNN). 
...
ggplotly(p.train) %>% layout(autosize = F, width = 900, height = 400)
ggplotly(p.test) %>% layout(autosize = F, width = 900, height = 400)
```
</div>
